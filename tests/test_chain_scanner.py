...
from app.services.options.chain_scanner.providers.base import (
    ChainProvider, ChainSnapshot, HistoryData, OptionContract,
)
from app.services.options.chain_scanner.providers.cached_provider import CachedProvider
from app.services.options.chain_scanner.iv_rank import compute_iv_metrics
from app.services.options.chain_scanner.contract_filter import filter_contracts
from app.services.options.chain_scanner.edge import compute_edge
from app.services.options.chain_scanner.scorer import score_signal, rank_signals
...
def _make_history(n_days=252, ticker='AAPL', base_price=175.0,
    ...
    returns = np.random.normal(0.0003, daily_vol, len(dates))
...
class TestProviderBase:
    ...
    def test_history_data_construction(self):
        ...
        assert len(h.returns) >= 250  # ~252 business days in a year
